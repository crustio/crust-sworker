    ...
    if(req.method() == http::verb::post)
    {
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            else
            {
                ...
                {
                    sprintf(ret_info_buffer, "Delete file '%s' failed! Invoke SGX API failed! Error code:%lx", cid.c_str(), sgx_status);
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            else
            {
                ...
                {
                    sprintf(ret_info_buffer, "Seal file '%s' failed! Invoke SGX API failed! Error code:%lx", cid.c_str(), sgx_status);
                    ret_info.append(ret_info_buffer);
                    p_log->err("%s\n", ret_info.c_str());
                    ret_code = 500;
                ...
                else if (CRUST_SUCCESS != crust_status)
                {
                    ...
                    case CRUST_STORAGE_FILE_DUP:
                        ...
                        break;
                    default:
                        sprintf(ret_info_buffer, "Seal file '%s' failed! Unexpected error, error code:%lx", cid.c_str(), crust_status);
                        ret_info.append(ret_info_buffer);
                        p_log->err("%s\n", ret_info.c_str());
                        ret_code = 500;
                    }
