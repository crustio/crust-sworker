...
/**
 * @description: Find substring in given byte array
 * @param src -> Pointer to source byte array
 * @param src_len -> Source byte array length
 * @param dst -> Pointer to destination byte array
 * @param dst_len -> Destination byte array length
 * @param pos -> Start to search position
...
long memfind(const void *src, size_t src_len, const void *dst, size_t dst_len, size_t pos)
{
    ...
    const uint8_t *udst = (const uint8_t *)dst;
    for (; pos <= src_len; pos++)
    {
        if (usrc[pos] == udst[0])
        {
