...
static string ias_response_header[3] = {
...
{
    printf("[INFO] initial IAS request object, serverid:%d\n",server_idx);
...
ias_error_t IAS_Request::report(map<string,string> &payload, string &content,
	vector<string> &messages, vector<string> *response_v)
{
	Response response;
	...
	certchain= response.headers_as_string(ias_response_header[0]);
	sigstr= response.headers_as_string(ias_response_header[1]);
	content= response.content_string();

    response_v->push_back(ias_response_header[0]);
    response_v->push_back(certchain);
    response_v->push_back(ias_response_header[1]);
    response_v->push_back(sigstr);
    response_v->push_back(ias_response_header[2]);
    response_v->push_back(content);

	return IAS_OK;
}

ias_error_t IAS_Request::report_old(map<string,string> &payload, string &content,
	vector<string> &messages, vector<string> *response_v)
{
	Response response;
	map<string,string>::iterator imap;
    ...
    resData += certchain;
    if(response_v) {
        response_v->push_back("X-IASReport-Signing-Certificate");
        response_v->push_back(certchain);
    }
    ...
    resData += sigstr;
    if(response_v) {
        response_v->push_back("X-IASReport-Signature");
        response_v->push_back(sigstr);
    }
    ...
    /*if(pr != NULL) {
        eprintf("response size: %d\n", sizeof(response));
        memcpy(pr, resData.c_str(), resData.size());
        *prs = resData.size();
    }*/
