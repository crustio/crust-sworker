...
bool upgrade_try_start()
{
    ...
    while (true)
    {
        http::response<http::string_body> res_inform = client->Get(p_config->base_url + "/upgrade/start_test", "", headers);
    ...
    while (true)
    {
        ...
        if (UPGRADE_STATUS_END == ed->get_upgrade_status())
        {
            ...
            if (SGX_SUCCESS != (sgx_status = Ecall_gen_upgrade_data_test(global_eid, &crust_status, 
                            g_block_height+REPORT_BLOCK_HEIGHT_BASE+REPORT_INTERVAL_BLCOK_NUMBER_LOWER_LIMIT)))
            {
        ...
        if (UPGRADE_STATUS_EXIT == ed->get_upgrade_status())
        {
            g_block_height += REPORT_BLOCK_HEIGHT_BASE;
            // Release database
