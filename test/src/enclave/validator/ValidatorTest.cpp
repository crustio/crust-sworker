...
extern std::vector<uint8_t *> g_del_srd_v;
extern sgx_thread_mutex_t g_del_srd_v_mutex;
extern std::map<int, uint8_t *> g_validate_srd_m;
extern std::map<int, uint8_t *>::const_iterator g_validate_srd_m_iter;
extern sgx_thread_mutex_t g_validate_srd_m_iter_mutex;
extern bool g_validate_srd_finish;
extern sgx_thread_mutex_t g_validate_srd_finish_mutex;
// File related method and variables
extern crust_status_t validate_real_file(uint8_t *p_sealed_data, size_t sealed_data_size);
extern std::unordered_set<uint32_t> g_del_files_idx_us;
extern sgx_thread_mutex_t g_del_files_idx_us_mutex;
extern std::map<uint32_t, json::JSON> g_validate_files_m;
extern std::map<uint32_t, json::JSON>::const_iterator g_validate_files_m_iter;
extern sgx_thread_mutex_t g_validate_files_m_iter_mutex;
extern bool g_validate_files_finish;
extern sgx_thread_mutex_t g_validate_files_finish_mutex;
extern uint32_t g_validate_files_random;
extern uint32_t g_validate_random;
extern sgx_thread_mutex_t g_validate_random_mutex;
std::string oneg_cid = "";
json::JSON oneg_file;
size_t oneg_file_idx = 0;
...
void validate_srd_bench()
{
    ...
    while (g_validate_srd_m.size() > 0)
    {
        SafeLock sl(g_validate_srd_finish_mutex);
        sl.lock();
        if (g_validate_srd_finish)
        {
            break;
        }
        ...
        {
            sgx_thread_mutex_lock(&g_validate_srd_m_iter_mutex);
            std::vector<uint8_t *> tmp_del_srd_v;
            for ( ; g_validate_srd_m_iter != g_validate_srd_m.end(); g_validate_srd_m_iter++)
            {
                // Push current g_hash to delete buffer
                tmp_del_srd_v.push_back(g_validate_srd_m_iter->second);
                wl->add_srd_to_deleted_buffer(g_validate_srd_m_iter->first);
            }
            sgx_thread_mutex_unlock(&g_validate_srd_m_iter_mutex);
            // Add tmp deleted srd to real buffer
            sgx_thread_mutex_lock(&g_del_srd_v_mutex);
            g_del_srd_v.insert(g_del_srd_v.end(), tmp_del_srd_v.begin(), tmp_del_srd_v.end());
            sgx_thread_mutex_unlock(&g_del_srd_v_mutex);
            break;
        }
    }
...
void validate_srd_bench_real()
{
    ...
    g_validate_srd_m_iter++;
    auto cur_validate_srd_m_iter = g_validate_srd_m_iter;
    sl_iter.unlock();

    std::string g_hash = hexstring_safe(p_g_hash, HASH_LENGTH);

    Defer finish_defer([&cur_validate_srd_m_iter, &cur_validate_random, &deleted, &p_g_hash, &g_hash_index, &wl](void) {
        // Get current validate random
        sgx_thread_mutex_lock(&g_validate_random_mutex);
        ...
        {
            if (cur_validate_srd_m_iter == g_validate_srd_m.end())
            {
                sgx_thread_mutex_lock(&g_validate_srd_finish_mutex);
                g_validate_srd_finish = true;
                sgx_thread_mutex_unlock(&g_validate_srd_finish_mutex);
            }
            if (deleted)
            {
                sgx_thread_mutex_lock(&g_del_srd_v_mutex);
...
void validate_meaningful_file_bench()
{
    ...
    if (wl->get_report_file_flag())
    {
        ...
        size_t wait_time = 0;
        size_t timeout = REPORT_SLOT * BLOCK_INTERVAL * 1000000;
        while (g_validate_files_m.size() > 0)
        {
            SafeLock sl(g_validate_files_finish_mutex);
            sl.lock();
            if (g_validate_files_finish)
            {
                break;
            }
            ocall_usleep(wait_interval);
            ...
            {
                sgx_thread_mutex_lock(&g_validate_files_m_iter_mutex);
                std::unordered_set<uint32_t> tmp_del_files_idx_us;
                for ( ; g_validate_files_m_iter != g_validate_files_m.end(); g_validate_files_m_iter++)
                {
                    // Add file to deleted buffer
                    tmp_del_files_idx_us.insert(g_validate_files_m_iter->first);
                }
                sgx_thread_mutex_unlock(&g_validate_files_m_iter_mutex);
                // Add tmp deleted file to real buffer
                sgx_thread_mutex_lock(&g_del_files_idx_us_mutex);
                g_del_files_idx_us.insert(tmp_del_files_idx_us.begin(), tmp_del_files_idx_us.end());
                sgx_thread_mutex_unlock(&g_del_files_idx_us_mutex);
                break;
            }
        }
...
void validate_meaningful_file_bench_real()
{
    ...
    uint32_t file_idx = oneg_file_idx;
    json::JSON file = oneg_file;
    g_validate_files_m_iter++;
    auto cur_validate_files_m_iter = g_validate_files_m_iter;
    sl_iter.unlock();

    bool deleted = false;

    Defer finish_defer([&cur_validate_files_m_iter, &cur_validate_random, &deleted, &file_idx](void) {
        // Get current validate random
        sgx_thread_mutex_lock(&g_validate_random_mutex);
        ...
        {
            if (cur_validate_files_m_iter == g_validate_files_m.end())
            {
                sgx_thread_mutex_lock(&g_validate_files_finish_mutex);
                g_validate_files_finish = true;
                sgx_thread_mutex_unlock(&g_validate_files_finish_mutex);
            }
            if (deleted)
            {
                sgx_thread_mutex_lock(&g_del_files_idx_us_mutex);
